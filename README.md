...
| `onprem-handoff-{prod,stage}` | 온프렘 산출물 기록 — `secrets/.eks-control-plane-dns-ip` (DNS forwarder용) 와 `secrets/.argocd-cluster` (ArgoCD cluster 등록용 IAM 키·CA). `up-all`에 포함 |
...
| `onprem-handoff.sh` | 로컬 | terraform output을 읽어 환경별 (PROD_/STAGE_) 로 `secrets/.eks-control-plane-dns-ip` (pfSense DNS forwarder 안내 주석 포함) 와 `secrets/.argocd-cluster` (ArgoCD 전용 IAM 키·엔드포인트·CA — 600 권한) 를 기록합니다 |
...
- **핸드오프**: `up-all` (및 `onprem-handoff-*`) 이 두 산출물을 환경별로 기록합니다 — `secrets/.eks-control-plane-dns-ip` (pfSense conditional forwarder용 resolver inbound IP) 와 `secrets/.argocd-cluster` (ArgoCD cluster 등록용 IAM 키·엔드포인트·CA). 온프렘 ArgoCD는 후자를 `manifests/argocd/cluster-secret.yaml.example`에 채워 클러스터를 등록합니다 (절차는 `manifests/argocd/README.md`).
