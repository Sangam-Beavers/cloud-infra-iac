...
├── modules/          # vpc · kms · aurora · elasticache · eks · jumphost · vpn · route53-resolver · api-gateway · edge · cognito · cloudtrail · frontend-deploy-iam · argocd-iam
...
CONFIRM을 입력하면 다음 순서로 자동 진행되며, 완료 후 pfSense Peer Endpoint를 `exports/wireguard-{env}-eip`의 EIP로 설정하면 터널이 올라옵니다.
...
| `vpn-restart` / `vpn-eip` | 라우터 재기동 (키 반영, EIP 유지) / EIP를 `exports/wireguard-{env}-eip`에 기록 |
| `onprem-handoff-{prod,stage}` | 온프렘 DNS 산출물 기록 — `exports/eks-cp-{env}-dns-ip` (pfSense DNS forwarder용). `up-all`에 포함 (ArgoCD cluster Secret은 apply가 생성 — 아래) |
| `kubeconfig-{prod,stage}` | EKS kubeconfig 설정 — 온프렘/Tailscale 경유 직접 주소 (터널 불필요; VPN 끊기면 AWS 콘솔 Session Manager로 break-glass). **양쪽 모두 공유 admin 역할 `sb-{env}-eks-admin`을 assume** — `1000-leaders`(AdministratorAccess) 멤버면 ClusterAdmin, **MFA 강제**라 프로필에 `mfa_serial` 설정 필요 (최초 1회 인증 후 캐시). 역할/access entry는 apply로 생성됩니다 (prod는 현재 코드만 반영 — `apply-prod` 후 사용) |
...
| `onprem-handoff.sh` | 로컬 | terraform output을 읽어 `exports/eks-cp-{env}-dns-ip` (pfSense DNS forwarder 안내 주석 포함, 환경별 파일)를 기록합니다 (ArgoCD cluster Secret은 terraform `local_sensitive_file`이 apply 시 생성·destroy 시 삭제) |
...
- **버킷명은 자동**: `make state-bucket` (create-state-bucket.sh)이 계정의 기존 `global-bridge-tfstate*` 버킷을 발견해 재사용하거나, 없으면 `global-bridge-tfstate-<계정ID>`로 생성하고, 그 이름을 캐시 `exports/state-bucket-<profile>` (gitignore)에 기록합니다. make의 `STATE_BUCKET`은 그 캐시를 읽습니다 (cat — aws 호출 없음).
...
> 차트 버전은 `scripts/install-k8s-stack.sh`에 고정되어 있습니다 (재현성 · IAM 정책 버전 스큐 방지). 버전을 올릴 때는 ALB IAM 정책 (`modules/eks/policies/`)도 함께 갱신해야 합니다.

> **앱 서비스 IRSA** (애드온과 별개): `member`는 Cognito 관리 API (`modules/cognito`), `document`는 SQS/S3/Lambda (`modules/document-irsa`)를 SA IRSA로 호출합니다. SA (`sb-{env}-app-ns:{member,document}`)에 역할 ARN을 annotate하고, 앱 `build.gradle`에 `software.amazon.awssdk:sts`가 있어야 웹 아이덴티티 (AssumeRoleWithWebIdentity)가 해석됩니다 — 빠지면 조용히 노드 롤로 폴백하므로 주의해야 합니다. 별도로 `sb-stage-eks-admin` (팀 ClusterAdmin, MFA 강제)은 `kubeconfig-stage`가 assume합니다.

#### 6.1.5. 그린필드 부트스트랩 (vpc-cni → Cilium 교체)

EKS API가 private-only라 Terraform은 클러스터에 접근해 Cilium을 직접 깔 수 없습니다. 이 제약을 다음 순서로 우회합니다.

1. **EKS 기본 애드온으로 노드 Ready**: `cni = "cilium"`이어도 클러스터는 `bootstrap_self_managed_addons` 기본값 (true)으로 만들어, EKS가 기본 self-managed vpc-cni/kube-proxy를 깔아 노드를 Ready로 올립니다. 덕분에 terraform apply가 무인으로 완료됩니다 (Terraform은 vpc-cni/kube-proxy를 관리 애드온으로 채택하지 않을 뿐입니다).
...
- **흐름 2 (EKS 노드 → 온프렘 Harbor)**: private RT엔 Harbor `/32`만 라우터로 보내고 (`harbor_destinations`), 라우터가 터널 IP로 **SNAT (은닉)**하여 온프렘엔 단일 소스로만 보입니다 — private는 광고하지 않습니다. Route53 Resolver **outbound**로 Harbor 도메인을 온프렘 DNS로 포워딩합니다 (`dns_forward_domains`). (노드 containerd의 사설 CA 신뢰는 노드 측 설정 영역입니다.)
- **VPN 라우터**: 인터넷 경유 터널이라 WG `MTU = 1420`에 MSS clamp를 적용합니다 (점보프레임 8921이면 대용량 이미지 pull이 reset됩니다). forward SG는 흐름별 최소 포트만 엽니다 — private→Harbor TCP 443, mgmt→DNS UDP 53.
...
- **핸드오프**: `exports/eks-cp-{env}-dns-ip` (pfSense conditional forwarder용 resolver inbound IP)는 `onprem-handoff-*`이 기록하고, `exports/argocd-<env>-cluster.yaml` (ArgoCD에 바로 apply할 완성본 cluster Secret)는 **terraform `local_sensitive_file`이 apply 시 생성·destroy 시 삭제**합니다 (키 등 민감값을 포함하므로 `.frontend-deploy`와 동일하게 라이프사이클로 관리하며 권한은 600). 온프렘 ArgoCD (`devops-system` NS)에 `kubectl apply`하면 등록됩니다 (IAM이 네임스페이스 한정이라 Secret의 `namespaces`로 ArgoCD 관리 범위도 일치시킵니다).
