...
| `onprem-handoff-{prod,stage}` | 온프렘 DNS 산출물 기록 — `secrets/.eks-cp-{env}-dns-ip` (pfSense DNS forwarder용). `up-all`에 포함 (ArgoCD cluster Secret은 apply가 생성 — 아래) |
...
- **핸드오프**: `secrets/.eks-cp-{env}-dns-ip` (pfSense conditional forwarder용 resolver inbound IP)는 `onprem-handoff-*`이 기록하고, `secrets/.argocd-<env>-cluster.yaml` (ArgoCD에 바로 apply할 완성본 cluster Secret)는 **terraform `local_sensitive_file`이 apply 시 생성·destroy 시 삭제**합니다 (키 등 민감값 포함이라 `.frontend-deploy`와 같이 라이프사이클 관리, 600). 온프렘 ArgoCD (`devops-system` NS) 에 `kubectl apply` 하면 등록됩니다 (IAM이 네임스페이스 한정이라 Secret의 `namespaces`로 ArgoCD 관리 범위도 일치시킴).
