...
| `kubeconfig-{prod,stage}` | EKS kubeconfig 설정 — 온프렘/Tailscale 경유 직접 주소 (터널 불필요; VPN 끊기면 AWS 콘솔 Session Manager로 break-glass). **양쪽 모두 공유 admin 역할 `sb-{env}-eks-admin`을 assume** — `1000-leaders`(AdministratorAccess) 멤버면 ClusterAdmin, **MFA 강제**라 프로필에 `mfa_serial` 설정 필요 (최초 1회 인증 후 캐시). 역할/access entry는 apply로 생성됨 (prod는 현재 코드만 반영 — `apply-prod` 후 사용) |
...
> **앱 서비스 IRSA** (애드온과 별개): `member`는 Cognito 관리 API (`modules/cognito`), `document`는 SQS/S3/Lambda (`modules/document-irsa`)를 SA IRSA로 호출합니다. SA (`sb-{env}-app-ns:{member,document}`)에 역할 ARN을 annotate하고, 앱 `build.gradle`에 `software.amazon.awssdk:sts`가 있어야 웹아이덴티티(AssumeRoleWithWebIdentity)가 해석됩니다 — 없으면 조용히 노드롤로 폴백. 별도로 `sb-stage-eks-admin` (팀 ClusterAdmin, MFA 강제)은 `kubeconfig-stage`가 assume합니다.
...
- **핸드오프**: `exports/eks-cp-{env}-dns-ip` (pfSense conditional forwarder용 resolver inbound IP)는 `onprem-handoff-*`이 기록하고, `exports/argocd-<env>-cluster.yaml` (ArgoCD에 바로 apply할 완성본 cluster Secret)는 **terraform `local_sensitive_file`이 apply 시 생성·destroy 시 삭제**합니다 (키 등 민감값 포함이라 `.frontend-deploy`와 같이 라이프사이클 관리, 600). 온프렘 ArgoCD (`devops-system` NS) 에 `kubectl apply` 하면 등록됩니다 (IAM이 네임스페이스 한정이라 Secret의 `namespaces`로 ArgoCD 관리 범위도 일치시킴).
