...
Terraform ≥ 1.15, AWS CLI, jq, make, session-manager-plugin이 필요합니다. EKS 내부 스택 (Cilium/ALB/ESO) 설치에는 `helm`, `kubectl`도 필요합니다 (`up-all`이 점프호스트 터널 경유로 호출).
...
CONFIRM을 입력하면 app → 환경 병렬 apply → **k8s 스택 (Cilium → ALB Controller → ESO)** → 부트스트랩 → VPN 키 등록·재기동 → EIP 기록까지 자동으로 진행됩니다. `cni = "cilium"`이라 EKS는 vpc-cni를 설치하지 않으므로, apply 직후 k8s 스택 단계가 Cilium을 올려 노드 네트워킹을 완성합니다 (이 단계 없이는 노드가 NotReady). 완료 후 pfSense Peer Endpoint를 `secrets/eip.env`의 EIP로 설정하면 터널이 올라옵니다.

> k8s 스택 단계가 터널/Helm 문제로 실패하면 인프라는 그대로 두고 그 단계만 재실행하면 됩니다: `make k8s-stack-prod PHASE=all && make k8s-stack-stage PHASE=all` (포트 공유로 직렬). 단계 인자로 `PHASE=cilium|alb|eso`를 줘 개별 재시도도 가능합니다.
...
k8s 스택은 kubectl 터널 ([4](#4-명령어-레퍼런스)의 `kubectl-tunnel-*`) 을 연 뒤 확인합니다 — `kubectl get pods -A`에서 `cilium`/`aws-load-balancer-controller`/`external-secrets` 파드가 Running이고, 노드가 모두 Ready면 정상입니다.
...
3. `up-all`은 환경 apply 직후 k8s 스택 (Cilium/ALB/ESO) 까지 설치합니다 — `cni = "cilium"`이라 이 단계가 없으면 노드가 NotReady로 남습니다. 이 단계만 따로 돌리려면 `make k8s-stack-{prod,stage} PHASE=all` ([4](#4-명령어-레퍼런스) 참고).

4. 클러스터에 **앱 Ingress / LoadBalancer 타입 Service**가 떠 있으면, ALB Controller가 만든 ALB/NLB는 Terraform이 모릅니다. `down-all` (terraform destroy) 전에 `kubectl delete ingress,svc --all -A`로 먼저 회수해야 합니다 — 안 그러면 ALB·관련 SG가 남아 VPC 삭제를 막습니다. Cilium 보조 ENI는 `DeleteOnTermination=true`라 노드 종료 시 자동 정리됩니다.
...
**그린필드 동작 (vpc-cni 부트스트랩 → Cilium 교체)**: EKS API는 private-only라 Terraform이 클러스터에 접근해 Cilium을 깔 수 없습니다. 그래서 `cni = "cilium"`이어도 클러스터는 `bootstrap_self_managed_addons` 기본값 (true) 으로 만들어, EKS가 **기본 self-managed vpc-cni/kube-proxy를 깔아 노드를 Ready로** 만듭니다 → terraform apply가 무인으로 완료됩니다 (Terraform은 vpc-cni/kube-proxy를 관리 애드온으로 채택만 안 함). 이어서 `install-k8s-stack.sh`가 점프호스트 터널로 그 기본 vpc-cni/kube-proxy DaemonSet을 지우고 **Cilium으로 교체**합니다. 이 방식이라 노드가 NotReady로 매니지드 노드그룹 생성이 데드락되지 않습니다 (`bootstrap_self_managed_addons = false`로 두면 정확히 그 데드락이 발생).
