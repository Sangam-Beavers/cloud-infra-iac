...
4. **부트스트랩 · VPN 키 등록 · 재기동 · EIP 기록**: Redis AUTH와 논리 DB를 만들고, WG 키를 새 CMK로 등록한 뒤 라우터를 재기동하고 EIP를 기록합니다.
...
| `onprem-handoff-{prod,stage}` | EKS endpoint·resolver inbound IP를 `secrets/.eks-control-plane-dns-ip`에 기록 (온프렘 DNS forwarder 설정용 — `up-all`에 포함) |
...
| `onprem-handoff.sh` | 로컬 | EKS endpoint·resolver inbound IP를 terraform output에서 읽어 `secrets/.eks-control-plane-dns-ip`에 환경별 (PROD_/STAGE_) 기록합니다 — 온프렘 pfSense DNS forwarder 설정 안내 주석 포함 |
...
EKS API는 **private-only**입니다. 평소 접근은 `make kubeconfig-*` (온프렘 연동 시 직접 주소로 kubeconfig 설정)이고, VPN/온프렘이 끊긴 비상시엔 **AWS 콘솔 → Systems Manager → Session Manager로 점프호스트 접속** 후 kubectl을 break-glass로 씁니다 (CLI 터널 스크립트는 두지 않음).
...
2. **Cilium으로 교체**: 이어서 `install-k8s-stack.sh`가 점프호스트 터널로 그 기본 vpc-cni/kube-proxy DaemonSet을 지우고 Cilium으로 교체합니다.
...
- **흐름 1 (온프렘 ArgoCD → EKS API)**: 컨트롤플레인 ENI를 **mgmt 서브넷**에 두고 (`control_plane_subnet_ids`), Route53 Resolver **inbound**로 온프렘이 private 엔드포인트 호스트명을 해석합니다. EKS SG는 pfSense NAT IP (`argocd_source_cidrs`)만 443 허용, 인증은 access entry (`argocd_principal_arn`).
...
- **핸드오프**: `up-all`이 `secrets/.eks-control-plane-dns-ip`에 resolver inbound IP를 환경별로 기록 — 온프렘 pfSense에서 이 IP로 EKS 도메인 conditional forwarder를 겁니다.
