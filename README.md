...
> *커밋되지 않는 입력 두 가지 (tfvars, WG 키)와 배포 대상 계정만 준비하면 됩니다.*
...
3. **WG 키 배치**: `secrets/wg.prod.env`, `secrets/wg.stg.env`에 WireGuard 키 파일을 둡니다 (팀 비밀 채널로 수령).
...
2. **k8s 스택**: kubectl 터널 ([4](#4-명령어-레퍼런스)의 `kubectl-tunnel-*`)을 연 뒤 `kubectl get pods -A`에서 `cilium`/`aws-load-balancer-controller`/`external-secrets` 파드가 Running이고 노드가 모두 Ready면 정상입니다.
...
**그린필드 동작 (vpc-cni 부트스트랩 → Cilium 교체)**
