...
| `install-k8s-stack.sh` | 로컬 | 점프 호스트 SSM 터널 경유로 Cilium (CNI) · AWS Load Balancer Controller · External Secrets Operator를 Helm으로 설치합니다 — 클러스터 안엔 사전 설치물이 없어 로컬 `helm`/`kubectl`만 필요합니다 (`<prod\|stage> [cilium\|alb\|eso\|all]`) |
