...
| `onprem-handoff-{prod,stage}` | 온프렘 산출물 기록 — `secrets/.eks-control-plane-dns-ip` (DNS forwarder용) 와 `secrets/.argocd-<env>-cluster.yaml` (ArgoCD에 바로 apply할 cluster Secret). `up-all`에 포함 |
...
| `onprem-handoff.sh` | 로컬 | terraform output을 읽어 `secrets/.eks-control-plane-dns-ip` (pfSense DNS forwarder 안내 주석 포함, 환경별 PROD_/STAGE_) 와 `secrets/.argocd-<env>-cluster.yaml` (온프렘 ArgoCD devops-system NS에 바로 apply할 cluster Secret, 600 권한) 를 기록합니다 |
...
- **핸드오프**: `up-all` (및 `onprem-handoff-*`) 이 두 산출물을 환경별로 기록합니다 — `secrets/.eks-control-plane-dns-ip` (pfSense conditional forwarder용 resolver inbound IP) 와 `secrets/.argocd-<env>-cluster.yaml` (ArgoCD에 바로 apply할 완성본 cluster Secret — 구조는 `secrets/argocd-cluster.yaml.example`). 온프렘 ArgoCD (`devops-system` NS) 에 `kubectl apply` 하면 등록됩니다 (IAM이 네임스페이스 한정이라 Secret의 `namespaces`로 ArgoCD 관리 범위도 일치시킴).
