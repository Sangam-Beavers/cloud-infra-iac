...
ensure_app_namespaces() {
  ...
  # member-service Cognito IRSA — SA 'member'(앱 ns)이 cognito 관리 역할(AdminCreateUser 등)을 assume한다.
  # 차트 Deployment의 serviceAccountName: member 와 일치해야 함(stage). 역할 ARN은 cognito 모듈 출력(없으면 생략).
