...
# ---------------------------------------------------------------------------
# document-service IRSA — member 패턴(OIDC 동적 참조 + sub/aud 조건). 액세스 키 없이 SA로:
#   SQS(분석결과 consume) + S3(원본 presign PUT/조회·마스킹본 조회) + Lambda(챗봇 호출).
# 대상 리소스는 IaC 범위 밖(app/AI팀 소유)이고 동일계정에 존재 — ARN은 caller identity로
# 동적 구성해 계정 ID를 하드코딩하지 않는다(퍼블릭 안전).
# ---------------------------------------------------------------------------
locals {
...
resource "aws_iam_role_policy" "this" {
  ...
  # 권한 내용(statement)은 policies/document-irsa-policy.json.tftpl로 분리 — .tf는 ARN 주입만.
  # (modules/eks의 alb-controller-iam-policy.json 선례. 동적 ARN이라 file 대신 templatefile)
