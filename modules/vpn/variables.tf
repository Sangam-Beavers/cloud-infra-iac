...
variable "wg_mtu" {
  description = "WireGuard 인터페이스 MTU입니다. EC2는 점보프레임 (9001)이라 wg-quick 자동값이 8921이 되는데, 터널은 인터넷 (~1500)을 지나므로 큰 패킷이 fragment/drop됩니다. 인터넷 경유 터널은 1420을 권장합니다 (1500에서 WG 오버헤드를 뺀 값)."
  type        = number
  default     = 1420
}
...
variable "tunnels" {
  description = "WG 터널 정의 — 키는 인터페이스 이름 (wg0active/wg1standby)을 그대로 사용합니다"
  type = map(object({
...
# --- EKS 노드 → on-prem Harbor — private 대역은 광고하지 않고 SNAT로 숨깁니다 ---
variable "app_route_table_ids" {
  description = "app (private) 라우트 테이블입니다. app_onprem_destinations만 이 라우터 ENI로 보냅니다 (Harbor 한정). 비우면 설정하지 않습니다."
  type        = list(string)
  default     = []
}

variable "app_onprem_destinations" {
  description = "private 서브넷이 on-prem에서 닿을 목적지 CIDR (예: Harbor [\"10.0.0.10/32\"]). least-privilege로 /32 를 권장합니다"
  type        = list(string)
  default     = []
}

variable "snat_source_cidrs" {
  description = "on-prem으로 나갈 때 라우터 터널 IP로 SNAT (은닉)할 소스 CIDR입니다 (private 서브넷들). 비우면 SNAT하지 않습니다."
  type        = list(string)
  default     = []
}

# forward 트래픽 SG ingress — 흐름별로 최소 포트만 허용합니다. 허용하지 않으면 ENI에서 drop되어 AWS→on-prem 경로가 막힙니다.
